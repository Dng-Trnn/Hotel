...
namespace Manager_Hotel.ClassLoin
{
    class Modify
    {
        ...
        SqlConnection connection;//sử dụng lớp SqlConnection để tạo chuỗi kết nối
        SqlDataAdapter sqlDataAdapter=new SqlDataAdapter(); //tạo mới một instance SqlDataAdapter với câu lệnh lấy dữ liệu trong bảng
        DataTable table = new DataTable(); //Sử dụng DataTable để lấy dữ liệu và điền vào table 
        SqlDataReader dataReader; // doc dL trong ban

        public List<TaiKhoan> TaiKhoans(string query)
        {
            List<TaiKhoan> taiKhoans = new List<TaiKhoan>();//Tạo một danh sách tài khoản

            using (SqlConnection sqlConnection = Connection.GetSqlConnection())
            {
                sqlConnection.Open();
                sqlCommand = new SqlCommand(query, sqlConnection);// viết câu lệnh query để lấy dữ liệu
